...
#define EFI_PARAM_ATTR_APCOVE        1
...
ConvertToConfidentialMemory (IN UINT64 BaseAddr, IN UINT32 NumPage)
{
  ...
  Ret = SbiCoVHConvertPages (BaseAddr, NumPage);
  ...
  )
{
  ...
  BootInfo->Header.Attr = EFI_PARAM_ATTR_APCOVE;
...
StandaloneMmInitialization (IN OUT UINT64 *TvmGuestId)
{
  ...
  // Check TSM info
  Ret = SbiCoVHGetTsmInfo ((UINT64)&TsmInfo, sizeof (TsmInfo));
  if (Ret.Error != SBI_COVE_SUCCESS) {
  ...
  //
  // |---------------------------------------------------------------------------------------------- |
  // | TvmState | Page Tables | Boot Info + Stack + Heap + FV | TVM page zero Memory |
  // |---------------------------------------------------------------------------------------------- |
  //
  ...
  Ret = SbiCoVHCreateTvm ((UINT64)&TvmCreateParams, sizeof (TvmCreateParams));
  ...
  *TvmGuestId = Ret.Value;
  ...
  Ret = SbiCoVHCreateTvmVcpu (*TvmGuestId,
                                  RISCV_COVE_VCPU_ID,
                                  PageStart);
  ASSERT (Ret.Error == SBI_COVE_SUCCESS);
  ...
  Ret = SbiCoVHAddTvmPageTablePages (*TvmGuestId, PageStart, NumPtePages);
  ASSERT (Ret.Error == SBI_COVE_SUCCESS);
  PageStart += NumPtePages * SIZE_4KB;

  // Add usable memory region for TVM
  MmTvmSize = MmSize - (PageStart - MmBase);
  Ret = SbiCoVHAddTvmMemoryRegion (*TvmGuestId, MM_VM_RAM_BASE, MmTvmSize);
  ASSERT (Ret.Error == SBI_COVE_SUCCESS);
  ...
  Ret = SbiCoVHAddTvmMeasuredPages (*TvmGuestId, (UINT64)BootInfo,
                                        PageStart,
  ...
  Ret = SbiCoVHAddTvmMeasuredPages (*TvmGuestId, PcdGet64 (PcdRiscVStandaloneMmFdBase),
                                        PageStart,
  ...
  // Finalize the TVM
  Ret = SbiCoVHFinalizeTvm (*TvmGuestId,
                                MM_VM_RAM_BASE + MM_VM_RAM_IMAGE_START_OFFSET,
                                MM_VM_RAM_BASE + MM_VM_BOOT_INFO_OFFSET);
  ASSERT (Ret.Error == SBI_COVE_SUCCESS);

  // Add page zeros to the rest of MM meory
  Ret = SbiCoVHAddTvmZeroPages (*TvmGuestId, PageStart,
