...
VOID
EFIAPI
DelegatedEventLoop (IN UINTN CpuId, IN UINTN ChannelId, IN UINT64 MmNsCommBufBase, IN UINT64 MmNsCommBufSize)
{
  EFI_STATUS  Status;
  UINTN BufferSize = 0;
  ...
  UINT64 PhysicalBase = MmNsCommBufBase & ~(EFI_PAGE_SIZE - 1);
  UINT32 PhysHiAddress = (UINT32)(PhysicalBase >> 32);
  UINT32 PhysLoAddress = (UINT32)(PhysicalBase & 0xFFFFFFFF);
  Status = SbiMpxySetShmem(PhysHiAddress, PhysLoAddress, (MmNsCommBufSize / EFI_PAGE_SIZE) * EFI_PAGE_SIZE);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "DelegatedEventLoop: "
  ...
  while (TRUE) {
    Status = SbiMpxySendMessage(ChannelId, 3, (VOID *)MmNsCommBufBase, BufferSize, NULL, NULL);
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
        "DelegatedEventLoop: "
  ...
  )
{
  ...
  DelegatedEventLoop (CpuId, PayloadBootInfo->MpxyChannelId, PayloadBootInfo->MmNsCommBufBase, PayloadBootInfo->MmNsCommBufSize);
}
