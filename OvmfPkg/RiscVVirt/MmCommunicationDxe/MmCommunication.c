...
//
// Mpxy Channel ID, value loaded from device tree
//
  ...
  )
{
  ...
  Status = SbiMpxySendMessage(gMpxyChannelId, 2, (VOID *)mNsCommBuffMemRegion.VirtualBase, BufferSize, (VOID *)mNsCommBuffMemRegion.VirtualBase, &ResponseLen);
  
  switch (Status) {
    case EFI_SUCCESS:
      ZeroMem (CommBufferVirtual, ResponseLen);
      CommunicateHeader = (EFI_MM_COMMUNICATE_HEADER *)(mNsCommBuffMemRegion.VirtualBase);
      ResponseLen        = CommunicateHeader->MessageLength +
                          sizeof (CommunicateHeader->HeaderGuid) +
                          sizeof (CommunicateHeader->MessageLength);
  ...
  )
{
  ...
  Status = SbiMpxySendMessage(gMpxyChannelId, 1, (VOID *)mNsCommBuffMemRegion.VirtualBase, 0, (VOID *)mNsCommBuffMemRegion.VirtualBase, &ResponseLen);
  if (Status == EFI_SUCCESS) {
    CopyMem (&MmVersion, (VOID *)mNsCommBuffMemRegion.VirtualBase, ResponseLen);
  ...
  )
{
  ...
  Status = GetMmMpxyChannelId(&gMpxyChannelId);
  if (EFI_ERROR (Status)) {
    goto ReturnErrorStatus;
  }

  if(FALSE == SbiMpxyShmemInitialized()) {
    UINT32 PhysHiAddress = (UINT32)(mNsCommBuffMemRegion.PhysicalBase >> 32);
    UINT32 PhysLoAddress = (UINT32)(mNsCommBuffMemRegion.PhysicalBase & 0xFFFFFFFF);
    Status = SbiMpxySetShmem(PhysHiAddress, PhysLoAddress, (mNsCommBuffMemRegion.Length / EFI_PAGE_SIZE) * EFI_PAGE_SIZE);
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
